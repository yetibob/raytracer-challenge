...
void test_matrix_multiply() {
   ...
   assert(matrix_compare(expected, p) == 0);
}
...
void test_matrix_multiply_tuple() {
    ...
    Tuple result = matrix_multiply_tuple(m ,t1);
    Tuple e = tuple_point(18, 24, 33);
    assert(tuple_compare(e, result) == 0);
}
...
void test_multiply_matrix_by_identity_matrix() {
    ...
    assert(matrix_compare(m, result) == 0);
}

void test_multiply_tuple_by_identity_matrix() {
    Tuple t = tuple_point(1, 2, 3);
    t[3] = 4;
    Tuple result = matrix_multiply_tuple(matrix_IdentityMatrix(), t);
    assert(tuple_compare(t, result) == 0);
}
...
void test_matrix_transpose_matrix() {
    ...
    assert(matrix_compare(e, transpose) == 0);
}

void test_matrix_transpose_identity_matrix() {
    Matrix *id = matrix_IdentityMatrix();
    assert(matrix_compare(id, matrix_transpose(id)) == 0);
}
...
void test_get_matrix_sub() {
    ...
    assert(matrix_compare(e1, sub1) == 0);
    ...
    assert(matrix_compare(e2, sub2) == 0);
}
...
void test_get_matrix_cofactor() {
    ...
    assert(equals(-12, matrix_minor1) == 0);
    double matrix_cofactor1 = matrix_cofactor(m, 0, 0);
    assert(equals(-12, matrix_cofactor1) == 0);

    double matrix_minor2 = matrix_minor(m, 1, 0);
    assert(equals(25, matrix_minor2) == 0);
    double matrix_cofactor2 = matrix_cofactor(m, 1, 0);
    assert(equals(-25, matrix_cofactor2) == 0);
}
...
void test_find_matrix_determinant_3_4_dim_matrix() {
    ...
    assert(equals(56,   matrix_cofactor(m3, 0, 0)) == 0);
    assert(equals(12,   matrix_cofactor(m3, 0, 1)) == 0);
    assert(equals(-46,  matrix_cofactor(m3, 0, 2)) == 0);
    assert(equals(-196, matrix_determinant(m3))    == 0);

    double arr2[] = { -2, -8, 3, 5, -3, 1, 7, 3, 1, 2, -9, 6, -6, 7, 7, -9 };
    Matrix *m4 = matrix_gen(arr2, 4);

    assert(equals(690,   matrix_cofactor(m4, 0, 0)) == 0);
    assert(equals(447,   matrix_cofactor(m4, 0, 1)) == 0);
    assert(equals(210,   matrix_cofactor(m4, 0, 2)) == 0);
    assert(equals(51,    matrix_cofactor(m4, 0, 3)) == 0);
    assert(equals(-4071, matrix_determinant(m4))    == 0);
}
...
void test_invert_matrix() {
    ...
    assert(equals(matrix_determinant(m1), 532) == 0);
    assert(equals(matrix_cofactor(m1, 2, 3), -160) == 0);
    assert(equals(m2->data[3][2], -160.0/532.0) == 0);
    assert(equals(matrix_cofactor(m1, 3, 2), 105) == 0);
...
void test_more_matrix_inversions() {
    ...
    assert(matrix_compare(e1, i1) == 0);
    assert(matrix_compare(e2, i2) == 0);
}
...
void test_multiply_product_by_its_matrix_inverse() {
    ...
    assert(matrix_compare(matrix_multiply(m3, matrix_inverse(m2)), m1) == 0);
}

void test_multiply_tuple_point_by_matrix_translation_matrix() {
    Matrix *m = matrix_translation(5, -3, 2);
    Tuple p = tuple_point(-3, 4, 5);
    Tuple transformed = matrix_multiply_tuple(m, p);

    Tuple expected = tuple_point(2, 1, 7);
    assert(tuple_compare(transformed, expected) == 0); 
}
...
void test_multiply_tuple_point_by_matrix_translation_matrix_inverse() {
    ...
    Tuple expected = tuple_point(-8, 7, 3);
    assert(tuple_compare(transformed, expected) == 0);
}

void test_multiply_tuple_vector_by_matrix_translation_does_nothing() {
    Matrix *m = matrix_translation(5, -3, 2);
    Tuple v = tuple_vector(-3, 4, 5);
    
    Tuple transformed = matrix_multiply_tuple(m, v);
    assert(tuple_compare(transformed, v) == 0);
}

void test_scale_matrix_applied_to_tuple_point() {
    Matrix *m = matrix_scaling(2, 3, 4);
    Tuple p = tuple_point(-4, 6, 8);
    Tuple scaled = matrix_multiply_tuple(m, p);
    Tuple expected = tuple_point(-8, 18, 32);
    assert(tuple_compare(expected, scaled) == 0);
}

void test_scale_matrix_applied_to_tuple_vector() {
    Matrix *m = matrix_scaling(2, 3, 4);
    Tuple v = tuple_vector(-4, 6, 8);
    Tuple scaled = matrix_multiply_tuple(m, v);
    Tuple expected = tuple_vector(-8, 18, 32);
    assert(tuple_compare(expected, scaled) == 0);
}
...
void test_multiply_by_matrix_inverse_of_matrix_scaling_matrix() {
    ...
    Tuple v = tuple_vector(-4, 6, 8);
    Tuple scaled = matrix_multiply_tuple(m, v);
    Tuple expected = tuple_vector(-2, 2, 2);
    assert(tuple_compare(expected, scaled) == 0);
}

void test_rotating_tuple_point_around_x_axis() {
    Tuple p = tuple_point(0, 1 , 0);
    Matrix *half = matrix_rotation_x(M_PI / 4);
    Matrix *full = matrix_rotation_x(M_PI / 2);
    Tuple rot1 = matrix_multiply_tuple(half, p);
    Tuple rot2 = matrix_multiply_tuple(full, p);

    Tuple e1 = tuple_point(0, sqrt(2)/2, sqrt(2)/2);
    Tuple e2 = tuple_point(0, 0, 1);
    assert(tuple_compare(e1, rot1) == 0);
    assert(tuple_compare(e2, rot2) == 0);
}
...
void test_rotating_tuple_point_around_y_axis() {
    Tuple p = tuple_point(0, 0, 1);
    Matrix *half = matrix_rotation_y(M_PI / 4);
    Matrix *full = matrix_rotation_y(M_PI / 2);
    Tuple rot1 = matrix_multiply_tuple(half, p);
    Tuple rot2 = matrix_multiply_tuple(full, p);

    Tuple e1 = tuple_point(sqrt(2)/2, 0, sqrt(2)/2);
    Tuple e2 = tuple_point(1, 0, 0);
    assert(tuple_compare(e1, rot1) == 0);
    assert(tuple_compare(e2, rot2) == 0);
}
...
void test_rotating_tuple_point_around_z_axis() {
    ...
    Tuple e1 = tuple_point(-sqrt(2)/2, sqrt(2)/2, 0);
    Tuple e2 = tuple_point(-1, 0, 0);
    assert(tuple_compare(e1, rot1) == 0);
    assert(tuple_compare(e2, rot2) == 0);
}
...
void test_matrix_shearing_moves_x_prop_y() {
    ...
    Tuple sheared = matrix_multiply_tuple(m, p);
    Tuple expected = tuple_point(5, 3, 4);
    assert(tuple_compare(expected, sheared) == 0);
}
...
void test_matrix_shearing_moves_x_prop_z() {
    ...
    Tuple p = tuple_point(2, 3, 4);
    Tuple sheared = matrix_multiply_tuple(m, p);
    Tuple expected = tuple_point(6, 3, 4);
    assert(tuple_compare(expected, sheared) == 0);
}
...
void test_matrix_shearing_moves_y_prop_x() {
    ...
    Tuple p = tuple_point(2, 3, 4);
    Tuple sheared = matrix_multiply_tuple(m, p);
    Tuple expected = tuple_point(2, 5, 4);
    assert(tuple_compare(expected, sheared) == 0);
}
...
void test_matrix_shearing_moves_y_prop_z() {
    ...
    Tuple p = tuple_point(2, 3, 4);
    Tuple sheared = matrix_multiply_tuple(m, p);
    Tuple expected = tuple_point(2, 7, 4);
    assert(tuple_compare(expected, sheared) == 0);
}
...
void test_matrix_shearing_moves_z_prop_x() {
    ...
    Tuple p = tuple_point(2, 3, 4);
    Tuple sheared = matrix_multiply_tuple(m, p);
    Tuple expected = tuple_point(2, 3, 6);
    assert(tuple_compare(expected, sheared) == 0);
}
...
void test_matrix_shearing_moves_z_prop_y() {
    ...
    Tuple p = tuple_point(2, 3, 4);
    Tuple sheared = matrix_multiply_tuple(m, p);
    Tuple expected = tuple_point(2, 3, 7);
    assert(tuple_compare(expected, sheared) == 0);
}

void test_individual_transformations_are_applied_in_sequence() {
    Tuple p = tuple_point(1, 0, 1);
    Matrix *rot = matrix_rotation_x(M_PI / 2);
    Matrix *scal = matrix_scaling(5, 5, 5);
    Matrix *trans = matrix_translation(10, 5, 7);
    
    Tuple p2 = matrix_multiply_tuple(rot, p);
    assert(tuple_compare(p2, tuple_point(1, -1, 0)) == 0);
    
    Tuple p3 = matrix_multiply_tuple(scal, p2);
    assert(tuple_compare(p3, tuple_point(5, -5, 0)) == 0);

    Tuple p4 = matrix_multiply_tuple(trans, p3);
    assert(tuple_compare(p4, tuple_point(15, 0, 7)) == 0);
}
...
void test_chained_transormations_must_be_applied_in_reverse() {
    ...
    Tuple transformed = matrix_multiply_tuple(t, p);
    assert(tuple_compare(transformed, tuple_point(15, 0, 7)) == 0);
}
